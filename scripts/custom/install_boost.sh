...
# Example usage:
#   ./install_boost.sh --version=1.74.0 --modules-dir=/path/to/modules --parallel=4
...
patch_dir="${script_dir}/patches/boost"
...
if version_le "${version}" '1.66'; then  # Boost <= 1.66.x
    ...
    patch -t -p1 < ${patch_dir}/1.66/boost_166-python37-unicode-as-string.patch
fi
...
if version_le "${version}" '1.64'; then  # Boost <= 1.64.x
    cd ${src_dir}
    patch -t -p2 < ${patch_dir}/1.64/boost_164-serialization-array-wrapper.patch
fi
...
if version_ge "${version}" '1.69' && version_le "${version}" '1.72'; then  # 1.69.x <= Boost <= 1.72.x
    cd ${src_dir}
    patch -t -p2 < ${patch_dir}/1.69/boost_169-pthread.patch
fi
