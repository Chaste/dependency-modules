...
# Check upstream dependency versions against infra/monitored-versions.json.
# Emits GitHub Actions outputs when GITHUB_OUTPUT is set.

usage()
{
  echo "Usage: $(basename "$0") [--manifest=path]"
  exit 1
}
...
check_dependency()
{
  ...
  if [ -z "${last_built}" ]; then
    echo "ERROR: missing last_built for ${dep} in ${manifest}" 1>&2
    exit 1
  fi
